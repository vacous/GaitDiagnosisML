...
const unsigned int PRINT_RATE = 0;
...
{  
  ...
  { 
    float Voltage_01 = analogRead(A0) * 5/1023.0;
    float Voltage_02 = analogRead(A2) * 5/1023.0;
    float voltage_01_actual = Voltage_01 - Voltage_02/10;
    float voltage_02_actual = Voltage_02 - Voltage_01/10;
    Serial.print(voltage_01_actual);
    Serial.print(" ");
    Serial.print(voltage_02_actual);
    Serial.print(" ");
    printAngles();
    lastPrint = millis();
  }
...
{ 
  ...
  float gravity = 9.81;
  float pi = 3.1415926;
  float ax = imu.calcAccel(imu.ax) * 9.81;
  float ay = imu.calcAccel(imu.ay) * 9.81;
  float az = imu.calcAccel(imu.az) * 9.81;
  float gx = imu.calcGyro(imu.gx) * pi/180;
  float gy = imu.calcGyro(imu.gy) * pi/180;
  float gz = imu.calcGyro(imu.gz) * pi/180;
